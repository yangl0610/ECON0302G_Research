...
EVENTS      = False        # True = 开启历史随机事件
RL_EPISODES = 120         # RL 训练轮数；设为 0 则跳过 RL，只跑规则策略
...
# ── 竞争激励（Multi-Agent Competitive Incentive）─────────────
# 开启后，每个 RL 智能体的奖励信号里会叠加一个竞争项：
#   comp = COMPETITIVE_WEIGHT × (本国GDP - 对手均值GDP) / 对手均值GDP
# 本国领先对手 → 正奖励；落后 → 负惩罚。
# 效果：智能体不只追求自身增长，还会对竞争态势做出响应，
#        涌现出更具攻击性或防御性的策略，适合做博弈论分析。
COMPETITIVE_RL     =  True  # True = 开启竞争激励
COMPETITIVE_WEIGHT = 0.25   # 竞争奖励项的权重（建议范围 0.1-0.5）
...
def run_rl(mode: str, seed: int, episodes: int,
           ...
           competitive: bool = False, competitive_weight: float = 0.25,
           events: bool = False):
    """
    训练 RL 智能体，然后跑一次最终模拟。
    ...
    参数：
      ...
      rl_agent_map       : {国家名: RL策略类型}；空则自动分配前 3 国
      competitive        : 开启竞争激励（COMPETITIVE_RL）
      competitive_weight : 竞争奖励项权重（COMPETITIVE_WEIGHT）
    返回：
      (rl_df, curves, engine)
    """
    civs         = build_competition_civs(mode, overrides or {})
    ...
    for i, civ in enumerate(civs):
        if civ.name in rl_agent_map:
            strat = make_strategy(rl_agent_map[civ.name])
        elif not rl_agent_map and i < len(default_rl):
            strat = make_strategy(default_rl[i])
        else:
        ...
        if isinstance(strat, QLearningStrategy):
            ...
            strat.competitive_weight = competitive_weight

        strategy_map[civ.name] = strat

    engine = SimulationEngine(
        civs=civs, strategy_map=strategy_map,
        events_enabled=events, seed=seed, training_mode=True,
    )
...
def save_comparison_chart(rule_df: pd.DataFrame, rl_df: pd.DataFrame,
    ...
    """
    生成规则策略 vs RL 的对比图（7 面板）。

    面板布局（4行 × 2列）：
      行0左：GDP 时序对比（rule 实线 / RL 虚线）
      行0右：Power（GDP×military×territories）时序对比
      行1左：Tech Composite 时序对比
      行1右：Trade Income 时序对比
      行2：  RL 技术选择热力图（全宽）
      行3左：RL 贸易政策热力图
      行3右：RL 扩张等级热力图
    ...
    def _plot_timeseries(ax, col, title, ylabel=None, compute_fn=None):
        """在 ax 上绘制 rule vs RL 时序线。compute_fn(row) 用于派生列（如 power）。"""
        for i, civ in enumerate(civs):
            color  = PALETTE[i % len(PALETTE)]
            d_rule = rule_df[rule_df["civilization"] == civ]
            d_rl   = rl_df[rl_df["civilization"] == civ]
            if compute_fn is not None:
                y_rule = d_rule.apply(compute_fn, axis=1)
                y_rl   = d_rl.apply(compute_fn, axis=1)
            else:
                y_rule = d_rule[col] if col in d_rule.columns else None
                y_rl   = d_rl[col]   if col in d_rl.columns   else None
            if y_rule is not None:
                ax.plot(d_rule["year"], y_rule, color=color, linewidth=2.0,
                        label=f"{civ} rule")
            if y_rl is not None:
                ls = "--" if civ in rl_civs else ":"
                ax.plot(d_rl["year"], y_rl, color=color, linewidth=1.4,
                        linestyle=ls, alpha=0.78,
                        label=f"{civ} RL" if civ in rl_civs else "_")
        for x1, x2, c in ERA_BANDS:
            ax.axvspan(x1, x2, alpha=0.05, color=c)
        ax.set_xlabel("Year", fontsize=8)
        ax.set_ylabel(ylabel or col, fontsize=8)
        ax.set_title(title, fontsize=9)
        ax.legend(fontsize=6.5, ncol=min(len(civs) * 2, 6), loc="upper left")
        ax.grid(alpha=0.22)

    # ── 行0：GDP | Power ────────────────────────────────────
    _plot_timeseries(fig.add_subplot(gs[0, 0]),
                     col="gdp", title=f"GDP  [{mode}]", ylabel="GDP")
    _plot_timeseries(fig.add_subplot(gs[0, 1]),
                     col=None,
                     title="Power (GDP × military × territories)",
                     ylabel="Power",
                     compute_fn=lambda r: r["gdp"] * r["military_str"] * r["territories"])

    # ── 行1：Tech Composite | Trade Income ──────────────────
    _plot_timeseries(fig.add_subplot(gs[1, 0]),
                     col="tech_composite", title="Tech Composite", ylabel="Tech")
    _plot_timeseries(fig.add_subplot(gs[1, 1]),
                     col="trade_income", title="Trade Income", ylabel="Trade Inc.")
    ...
    _label_axes(ax_tech, "RL Tech Focus  (0=Agri  1=Nav  2=Mil  3=Ind  4=Com)")
    ...
    im3 = ax_exp.imshow(mat_exp, aspect="auto", cmap="YlOrRd",
                         vmin=0, vmax=2, interpolation="nearest")
    _label_axes(ax_exp, "RL Expansion  (0=Stay  1=Moderate  2=Aggressive)")
...
if __name__ == "__main__":
    ...
    if rl_episodes > 0:
        ...
        if RL_OUTPUT.get("gdp_comparison", True):
            civs_sorted = sorted(rule_df["civilization"].unique())
            ...
            # ── 终局 Power 对比（GDP × military_str × territories）──
            print(f"\n{'═'*72}")
            print("Final Power — rule-based vs RL  (GDP × military × territories):")
            print(f"  {'Nation':<15} {'Rule':>11} {'RL':>11} {'Diff':>11}")
            print(f"  {_sep(52)}")
            for civ in civs_sorted:
                r_max = rule_df[rule_df["civilization"] == civ]["year"].max()
                l_max = rl_df[rl_df["civilization"] == civ]["year"].max()
                r_row = rule_df[(rule_df["civilization"]==civ) & (rule_df["year"]==r_max)].iloc[0]
                l_row = rl_df[(rl_df["civilization"]==civ)   & (rl_df["year"]==l_max)].iloc[0]
                r_pwr = r_row["gdp"] * r_row["military_str"] * r_row["territories"]
                l_pwr = l_row["gdp"] * l_row["military_str"] * l_row["territories"]
                diff  = l_pwr - r_pwr
                tag   = " ↑" if diff > 0.05 else (" ↓" if diff < -0.05 else "  ")
                print(f"  {civ:<15} {r_pwr:>11.2f} {l_pwr:>11.2f} {diff:>+11.2f}{tag}")
